...
<Tabs>
  ...
  ```kotlin
  // all DID methods require an instance of a key manager
...
<Tabs>
  ...
  //Import to create a did:dht with options 
  ...
  //Resolves DID
  val resolvedDid = DidResolvers.resolve(didDht.uri)

  //DID string
  val did = resolvedDid.didDocument.id.toString()
